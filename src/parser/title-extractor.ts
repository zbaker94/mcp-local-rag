...
type ExtractTitleParams = MarkdownParams | TxtParams | HtmlParams | PdfParams | DocxParams
...
/**
 * Extract title from a document based on its format
 *
 * This is the main entry point for title extraction. It dispatches to the
 * appropriate per-format extractor based on the format field.
 *
 * @param params - Format-specific parameters
 * @returns Title extraction result
 */
export function extractTitle(params: ExtractTitleParams): TitleExtractionResult {
  switch (params.format) {
    case 'md':
      return extractMarkdownTitle(params.text, params.fileName)
    case 'txt':
      return extractTxtTitle(params.text, params.fileName)
    case 'html':
      return extractHtmlTitle(params.readabilityTitle, params.fileName)
    case 'pdf':
      return extractPdfTitle(params.metadataTitle, params.firstPageItems, params.fileName)
    case 'docx':
      return extractDocxTitle(params.htmlContent, params.fileName)
  }
}
